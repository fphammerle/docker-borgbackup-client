...
IMAGE_NAME = docker.io/fphammerle/borgbackup-client
...
OPENSSH_CLIENT_PACKAGE_VERSION := $(shell grep -Po 'OPENSSH_CLIENT_PACKAGE_VERSION=\K.+' Dockerfile | tr -d _-)
ARCH = $(shell arch)
...
IMAGE_TAG = ${PROJECT_VERSION}-borgbackup${BORGBACKUP_PACKAGE_VERSION}-openssh${OPENSSH_CLIENT_PACKAGE_VERSION}-${IMAGE_TAG_ARCH}
